...
class _CalculatorState extends State<Calculator> {
  String output = '0'; // output merupakan variabel yang menampuung nilai yang akan ditampilkan dikalkulator

  void buttonPressed(String buttonText) { //fungsi buttonPressed yang akan dipanggil ketika tombol ditekan
    setState(() {
      if (buttonText == "C") { //jika c ditekan, output diubah menjaadi 0
        output = "0";
      } else if (buttonText == "=") { //jika tombol == ditekan, output diubah menjadi hasil dari expression
        try {
          output = evaluateExpression( // evaluateExpression adalah fungsi yang digunakan untuk menghitung hasil dari expression
              output.replaceAll('x', '*').replaceAll('÷', '/'));
        } catch (e) { //catch digunakan untuk menangani kesalahan yang terjadi ketika menghitung expression (contohnya jika user memasukkan fungsi ysng tidak falid seperti 1++2 atau +
          output = "Error"; //jika terjadi kesalahan, output yang muncul di kalkulator diubah menjadi eror
        }
      } else {
        if (output == "0") {
          output = buttonText;
  ...
  String evaluateExpression(String expression) { //fungsi ini digunakan untuk mengonversi expresion menjadi hasil perhitungan.fungsi ini diambil dari expression dart
  ...
  Widget buildButton(String buttonText, Color color,
      {double widthFactor = 1.0}) { //fungsi ini digunakan untuk membuat widget tombol dengan teks, warna,dan ukuran yang sudah ditentukan
    return Expanded( //membuat tombol dapat mengisi ruang horizontal yang tersedia dalam 1 baris atau 1 row
      flex: widthFactor.toInt(), //widthfactor digunakan untuk menentukan lebar tombol menjadi 1. flex digunakn untuk menentukan proporsi lebar tombol dalam baris
      child: Padding(
        padding: const EdgeInsets.all(8.0),
        child: ElevatedButton( //digunakan unruk membuat tombol degan gaya tertentu
          style: ElevatedButton.styleFrom( //elevated button itu button yang langsung ada bingkainya,sudah ada bentuk buttonnya atau fieldnya
            padding: EdgeInsets.symmetric(
                vertical: 22), // digunakan untuk mengatur jarak dalam tombol secara vertical menjadi 22 sehingga tombol terlihat tinggi
  ...
  Widget build(BuildContext context) { //fungsi build digunakan untuk membuat tampilan utama
    return Scaffold( //digunakannuntuk membuat struktur dasar untuk halaman
      body: Column(
        crossAxisAlignment: CrossAxisAlignment.stretch, //tanpa crossaxisaligmnet.stretch tombol2 tersebut akan meiliki lebar sesuai dengan ukuran teksnya masing masing atau pengaturan defauil widget tombol
        children: <Widget>[
          Expanded(
            child: Container(
